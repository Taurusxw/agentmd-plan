...
- 普通规则或 Skill 修改只运行 `verification-and-reporting.md` 为受影响面选择的检查；不得把所有可用验证命令当作每次必跑清单。
- 全局候选检查体量、版本、日期、语义覆盖和同步哈希；只有修改共享校验基础设施或正式发布时才运行完整 Skill 测试矩阵。
- 仅发布前运行秘密扫描、私有路径扫描、完整回归、`git diff --check` 和 GitHub 元数据检查。
